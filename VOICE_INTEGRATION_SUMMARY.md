...
- **Visual Feedback**: Shows "🎤 Listening: [transcript]..." while recording
...
- **Renamed Feature**: "RAG Chat" → "🤖 RAG ChatBot"
...
- Updated label from "RAG Chat" to "🤖 RAG ChatBot"
...
### 5. Cleanup
- ✅ Deleted "ChatBot - Transformer" folder as requested
- ✅ Integrated all useful functionality into main project
...
3. **Pause 2 Seconds** → Auto-submit query
4. **Or Click Stop** → Manual stop

### Voice Output
1. **Toggle Voice On/Off** → Control audio responses
2. **Automatic Speech** → AI reads responses aloud
3. **Natural Voice** → English voice with optimized settings
...
```
User clicks Mic → Browser requests permission → User speaks
    ↓
Real-time transcription shown → 2 seconds silence detected
    ↓
Auto-submit to RAG → SSE streaming response → Voice reads answer
    ↓
Sources displayed in dropdown → Conversation saved to history
```
